...
class FinanceModel extends \Core\Model
{
   ...
   public static function createPaymentData($customer_id,$order_id,$IBAN, $tnv)
   {
    ...
    public static function createSalePaymentData($order_id,$customer_id,$payment_type_id,$order_total)
    {

        try{
            $date=date('Y-m-d');
            $db=static::getDB();
            $stmt=$db->prepare('INSERT INTO forzaerp_retail_customer_payment (`order_id`,`customer_id`,`payment_type_id`,`order_total`,`date`) VALUES(?,?,?,?,?)');

            $stmt->execute([$order_id,$customer_id,$payment_type_id,$order_total,$date]);
            $message="payment data created";
            return $message;
